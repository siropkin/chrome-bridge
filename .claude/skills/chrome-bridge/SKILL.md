...
- `profiles` — list connected Chrome profiles. With several connected, a `<match>` routing to exactly one profile goes there automatically; a match in several profiles is REFUSED — name one with `--profile <id>` (prefix is enough). Never guess which profile the user meant: if it matters (personal vs work), ask.
