...
1. `health` — server down? Run `node <repo>/cli.mjs start` yourself (detached). Extension disconnected, or a stale-version stderr warning? Tell the user to reload it at `chrome://extensions`.
...
5. Done: `release` every tab you drove, `close` every tab you opened — the last Rules bullet is the checklist.
...
- `nav <match> <url>` / `open <url>` / `close <match>` / `release <match>` — tab lifecycle. `close` is only for tabs YOU opened; a tab you found is the user's — `release` it, don't close it. `open` warns on an exact-URL dupe; `nav` to the tab's current URL warns — that IS a reload.
- `scroll <match> down|up|bottom|@ref [--diff]` — scroll the page or an element into view; `--diff` shows what lazy-loaded in.
...
- `net <match> [--dur ms] [--filter s] [--body s] [--ws] [--har out.har]` — capture network, one line per request; `--dur` caps at 30s; `--ws` also captures WebSocket frames (chat/streaming apps); `--har` saves a shareable HAR 1.2. `fetch <match> <url> [--out file]` — replay/grab a URL in the page (the logged-in session rides it; binary → `--out`).
...
- `dialog <match> accept|dismiss` — answer a JS dialog, reachable only if it opened during a live debugger session (`net`/`shot`/…). One that wedged an unattached tab can't be answered — `nav <match> <url>` drops it and revives the tab.
...
- `net`/`shot`/`emulate`/`upload` attach Chrome's debugger while they run — detectable by page JS and anti-bot, and this is the user's real logged-in profile. Prefer `snap`/`eval`/`measure` when they answer.
