...
| `net <match> [--dur ms] [--filter s] [--body s] [--har out.har]` | Capture network traffic via CDP (≤30s per run) — one compact line per request, each naming its initiator (`⟵ api-client.js:88`); `--body s` appends matching JSON/text response bodies; `--har out.har` saves the capture as a shareable HAR 1.2 (opens in DevTools/Burp) |
