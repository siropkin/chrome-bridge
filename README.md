...
**Multiple Chrome profiles**: the extension can be loaded in several profiles at once — each keeps its own connection, and agents can drive them in parallel. A command routes to the only profile with a matching tab; a match in several profiles is **refused** until the agent names one with `--profile <id or name>` (`cli.mjs profiles` lists both) — the agent never silently acts in your personal browser when it meant the work one. Two bridge builds loaded in ONE Chrome (store + unpacked) are detected and refused loudly too — the ⏏ gate covers only one install, so the bridge tells you to unload one instead of pretending they're two profiles. Each profile also gets a stable short name (`birch`, `oak`, …) accepted by `--profile` and shown in `watch` lines and profile tags — a uuid prefix means nothing to the human watching. (The refusal is a safety net for honest CLI use, not a security boundary: any local process can set `profile` in a `/cmd` body and route anywhere — the same local trust model as before.)
...
| `click <match> <@ref\|css> [--dbl] [--diff] [--trusted]` | Click (scrolls into view, full pointer/mouse event sequence, overlay-coverage check; if nothing observably reacts, it says so instead of faking success); `--dbl` double-clicks; `--trusted` drives CDP Input — isTrusted=true, so canvas tools (Figma) accept it. Refs are identity-checked at click time — a page that re-sorted under a stale ref fails loudly instead of clicking the wrong element |
...
| `clear <match> <@ref\|css> [--diff]` | Empty an input/textarea/contenteditable — the path when synthetic select-all+Backspace can't (editor-owned models like Editor.js) |
| `activate <match>` | Bring a background tab to the front (trusted CDP input needs it — on a hidden tab it fires nothing) |
| `type <match> <@ref\|css> <text> [--diff] [--trusted]` · `press <match> <key> [@ref] [--diff] [--trusted]` · `hover <match> <@ref\|css> [--diff] [--trusted]` | Per-char typing (autocomplete UIs; paced and budget-scaled for heavy editors, follows the caret when the editor rewrites the block under it), key presses (`Control+k` combos work), hover; `--trusted` drives CDP Input (isTrusted=true — canvas tools accept it; Enter triggers browser defaults like form submit) |
| `paste <match> [@ref\|css] [--diff] [--html] [-- <text>]` | Real-paste semantics into the focused (or given) field — rich editors that revert `fill` (Quill, Reddit/LinkedIn composers) take a paste; without `-- <text>` it reads the OS clipboard. `--html` sends markup (text/html + stripped plain fallback) — block editors parse it into native blocks in one paste |
...
| `net <match> [--dur ms] [--filter s] [--body s] [--ws] [--har out.har]` | Capture network traffic via CDP (≤30s per run) — one compact line per request, each naming its initiator (`⟵ api-client.js:88`); `--ws` also captures WebSocket frames (`→` sent / `←` received — chat/streaming apps); `--body s` appends matching JSON/text response bodies; `--har out.har` saves the capture as a shareable HAR 1.2 (opens in DevTools/Burp) — it carries full headers incl. cookies/tokens, so it's written owner-only (0600): treat it as a secret |
| `fetch <match> <url> [--out file] [--keep] [--header "k: v"]... [--csrf]` | In-page fetch riding the logged-in session — login-walled JSON/feeds answer it without `eval` plumbing; binary responses need `--out`, text prints capped at 50K chars (`--out` gets the full body); no matching tab → a scratch tab on the target origin is opened, fetched, closed (`--keep` leaves it for follow-ups); `--header` adds request headers for session APIs that check them; `--csrf` attaches the JSESSIONID cookie as `csrf-token` |
...
| `doctor [--fix]` | Browser-hygiene sweep: tabs still wearing bridge markers (🟣 group, status favicon, emulation, debugger) that the bridge no longer tracks — each row annotated with the tab's last lifecycle events, so "why is this tab still marked?" has an answer. `--fix` releases every residue tab and prunes dead-id memory (driven tabs are never touched). Marker transitions are also journaled as `[tab]` lines in `server.log`, and dead-session residue is reaped automatically when the extension restarts |
