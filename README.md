...
| `net <match> [--dur ms] [--filter s] [--body s]` | Capture network traffic via CDP (≤30s per run) — one compact line per request, each naming its initiator (`⟵ api-client.js:88`); `--body s` appends matching JSON/text response bodies |
